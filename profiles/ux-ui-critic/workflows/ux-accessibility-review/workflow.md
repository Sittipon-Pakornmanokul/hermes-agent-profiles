...
1. Read original user goals, acceptance criteria, target viewports and actual design/image artifacts. Establish the journeys and evidence available for independent review.

2. Save a provisional critique early. Inspect visual hierarchy, messaging, consistency, interaction clarity and accessibility against the stated goals.

3. For decision-layer interfaces, check that proposed actions, validated execution, approval blockers, partial results and verified completion are distinguishable and truthful.

4. Use browser evidence when interaction, responsiveness or keyboard accessibility is in scope. State what static images cannot prove; code correctness belongs to code-reviewer.

5. Return evidence-backed findings with severity, user impact, artifact location, suggested repair and retest steps. Write review artifacts only; re-review affected journeys and never claim real-user validation from model agreement.
