...
- **Tests that pass through a schema gate** — when the code under test sits behind a schema/validator that drops unknown fields, a test planting a secret or payload in a dropped field passes no matter what the code does. Plant it where the schema keeps it (e.g. a dict key of an allowed field) and assert on persisted output, then neutralise the guard to prove the test fails.
